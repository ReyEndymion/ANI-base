...
export function makeWASocket(connectionOptions, options = {}) {
     ...
     */
    //const {opts} = options
    let conn = _makeWaSocket(connectionOptions)//opts['legacy'] ? makeWALegacySocket : 
...
END:VCARD
    ...
    const media = await prepareWAMessageMedia({[/video/.test(type.mime) ? 'video' : /image/.test(type.mime) ? 'image' : 'document']: buffer}, {upload: conn.waUploadToServer})
...
export function serialize() {
    ...
    const m = Object.defineProperties(proto.WebMessageInfo?.prototype, {
        ...
        isBaileys: {
            get() {
                return this.id?.length === 32 || this.id?.length === 16 || this.id?.startsWith('3EB0') && this.id?.length === 12 || false
            }
        },
        chat: {
            get() {
...
//console.log('simple: ', )optionssender, this.conn?.user.jid, this.conn.this
                return sender
            },
            enumerable: true
        },
