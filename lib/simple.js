...
return { jid: pn ? pn+S_WHATSAPP_NET : null, lid: id, id: value, pn: pn || null, type: 'lid' }
}
...
//, devices.map(d => (0, jidEncode)(d.user, isLid ? 'lid' : 's.whatsapp.net', d.device))
...
const jid = (0, jidEncode)(isMe && isLid ? ((_g = (_f = conn.authState.creds) === null || _f === void 0 ? void 0 : _f.me) === null || _g === void 0 ? void 0 : _g.lid.split(':')[0]) || user : user, isLid ? 'lid' : 's.whatsapp.net', device);
//const jid = (0, jidEncode)(isMe && isLid ? ((_b = (_a = conn.authState.creds) === null || _a === void 0 ? void 0 : _a.me) === null || _b === void 0 ? void 0 : _b.lid.split(':')[0]) || user : user, isLid ? 'lid' : 's.whatsapp.net', device);
if (isMe) {
meJids.push(jid);
}
else {
...
} else if (message.interactiveMessage?.shopStorefrontMessage) {
return 'shop';
} else if (message.interactiveMessage?.collectionMessage) {
return 'collection';
...
if (connectionOptions.emitOwnEvents) {
process.nextTick(() => {
...
buffer = type.data
media = await prepareWAMessageMedia({[/video/.test(type.mime) ? 'video' : /image/.test(type.mime) ? 'image' : 'document']: buffer}, {upload: conn.waUploadToServer})
} catch { 
buffer = null 
}
}
...
header: proto.Message.InteractiveMessage.Header.create(Buffer.isBuffer(buffer) ?
...
...media,
headerType: /video/.test(type.mime) ? 3 : /image/.test(type.mime) ? 4 : 2,
...
//buttonId: !nullish(btn[1]) && btn[1] || !nullish(btn[0]) && btn[0] || '',([/video/.test(type.mime) ? 'videoMessage' : /image/.test(type.mime) ? 'imageMessage' : 'documentMessage']: )
//buttonText: {displayText: !nullish(btn[0]) && btn[0] || !nullish(btn[1]) && btn[1] || ''}buffer
}}),
messageParamsJson: '',
}),
contextInfo: {
...
return conn.sendInteractiveResponse(jid, message, quoted)
...
let type, media
console.log('ANIRENOV: ', listSections)//conn.DisconnectReason, makeWASocket type let  = objs//await import('../config.js')objsobjs, {[/video/.test(type.mime) ? 'video' : /image/.test(type.mime) ? 'image' : 'document']: buffer}JSON.stringify(listMessage)
if (buffer) try { 
type = await conn.getFile(buffer)
buffer = type.data 
media = await prepareWAMessageMedia({[/video/.test(type.mime) ? 'video' : /image/.test(type.mime) ? 'image' : 'document']: buffer}, {upload: conn.waUploadToServer})
...
title: !nullish(title) && title || !nullish(title) && title || '',
...
return conn.sendInteractiveResponse(jid, listMessage, quoted)
//return await conn.sendMessage(jid, listMessage, {quoted, upload: conn.waUploadToServer, ...options})
}
},        
cMod: {
/**
...
const userNumbers = allTags.filter(([, , g]) => !g).map(([match, number]) => {
try {
//const resolveUser = conn.resolveUser(number+lid)
//let id = isLidUser(number+lid) ? lid : userID 
return number + '@lid'
} catch (error) {
}
...
resolve(v.name || v.subject || (jid.endsWith('@s.whatsapp.net') && isPnUser(jid) ? parsePhoneNumber('+' + jid.split('@')[0]).number.international : jid.split('@')[0]))
})
} else {
...
const name = (withoutContact ? '' : v.name) || v.name || v.subject || v.vname || v.notify || v.verifiedName
return name || (jid.endsWith('@s.whatsapp.net') && isPnUser(jid) ? parsePhoneNumber('+' + jid.split('@')[0]).number.international : jid.split('@')[0]);
},
enumerable: true
},
loadMessage: {
...
break;
default: {
console.log(`${conn.chats[chat] ? conn.chats[chat].subject : chat}`, {
...
const m = Object.defineProperties(proto.WebMessageInfo?.prototype, {
conn: {
value: undefined,
enumerable: false,
writable: true,
...
isGroup: {
get() {
return isJidGroup(this.key.remoteJid);
},
enumerable: true,
},
chat: {
...
return (
this.key?.remoteJid || (senderKeyDistributionMessage && senderKeyDistributionMessage !== 'status@broadcast') || '').decodeJid();
}
},
sender: {
get() {//this.key?.fromMe &&Pn || this.chat || ''this.conn?.decodeJid()
const sender = this.isGroup ? this.key.fromMe ? jidNormalizedUser(this.conn.user.lid) : this.key.participant || this.key?.participantAlt || this.key?.participantPn || this.participant : this.chat
return sender
},
enumerable: true
},
...
console.log('simple: ', this.chat)//optionssender, this.conn?.user.jid, this.conn.this
...
return sender ? await self.conn?.getName(sender) : null;
},
enumerable: true,
},
vM: {
