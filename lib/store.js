 ...
 * @type {import('baileys')}
 */
import { initAuthCreds, BufferJSON, proto, isJidBroadcast, isJidNewsletter, isJidGroup, isLidUser, isPnUser, WAMessageStubType, useMultiFileAuthState as baileysMultiFileAuthState, getHistoryMsg, getAggregateVotesInPollMessage, jidNormalizedUser } from 'baileys'
...
*/
function events(sock, opts) {
const sockUserLid = jidNormalizedUser(sock.user.lid)//?.replace(/:\d+(?=@lid$)/, '')
...
Object.assign(chats[id], sock.chats[id])
}
})
...
if (isGroup && !chats[id].metadata) await fetchGroupMetadata(id, sock.groupMetadata)
Object.assign(chats, sock.chats)
}))
})
...
sock.handler(messagesUpsert)//sock.ev.on('messages.upsert', )
...
sock.groupsUpdate(groupsUpdate)//sock.ev.on('groups.update', )
...
if (!chats[id].metadata) await fetchGroupMetadata(id, sock.groupMetadata)
...
sock.participantsUpdate(groupParticipantsUpdate)//sock.ev.on('group-participants.update', )
// @ts-ignore
const id = groupParticipantsUpdate.id?.decodeJid?.()
if (!id || !isJidGroup(id)) return
if (!(id in chats) || !chats[id].metadata) await fetchGroupMetadata(id, sock.groupMetadata)
