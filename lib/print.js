...
import { __filename } from '../functions.js';


export default async function (m, conn = { user: {} }, options) {
    const {opts, db} = conn
const connUserLid = jidNormalizedUser(conn.user.lid)
const urlRegex = (await import('url-regex-safe')).default({ strict: false })
const terminalImage = opts['img'] ? require('terminal-image') : ''
if (/CIPHERTEXT/i.test(WAMessageStubType[m.messageStubType])) return
let _name = await conn.getName(m.sender)
let sender = parsePhoneNumber('+' + m.sender.split('@')[0]).number.international + (_name ? ' ~' + _name : '')
...
let user = conn.db.data[connUserLid].users[m.sender]
let me = parsePhoneNumber('+' + (conn.user?.jid).replace('@s.whatsapp.net', '')).number.international
